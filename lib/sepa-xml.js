...
function SepaXML(config = {}) {
  config = !config || typeof config !== 'object' ? {} : config;
  this.outputFormat = config.format || 'pain.001.001.03';

  const isPresent = config.ibanValidator && typeof config.ibanValidator.isValid === 'function';
  this.ibanValidator = isPresent ? ibanValidator : { isValid: (iban) => true };
...
SepaXML.prototype = {
  ...
  addTransaction: function(payment) {
    if (!payment || !payment.iban || !payment.name || payment.amount < 0 || !payment.id) return false;
    if (!this.ibanValidator.isValid(payment.iban)) return false;
